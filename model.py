...
class Model(object):
    ...
    def _conv(self, x, c):
        ...
        x = tflearn.layers.conv.conv_2d(x, c['num_filters'], c['filter_size'],
                c['stride'], activation='linear',
                regularizer='L2', weight_decay=c['l2'],
                weights_init=tfi.xavier(), padding=c['padding'],
                name='conv%d' %(self.conv_count))
    ...
    def _fully_connected(self, x, c):
        if 'activation' not in c:
            c['activation'] = 'linear'
        x = tflearn.fully_connected(x, c['num_outputs'], activation=c['activation'],
                weight_decay=c['l2'], name='fully_connncted%d' % self.fc_count)
    ...
    def _build_model(self, is_train):
        ...
        with tf.variable_scope('net', reuse=not is_train):
            ...
            x = self._conv_relu_bn(x, {'num_filters': 4*nf, 'filter_size': 3,
                    'stride': 1, 'padding': 'same', 'l2': l2, 'is_train': is_train})
            # x = self._pool(x, {'filter_size': 20, 'mode': 'max'})
            x = self._global_pool(x, {'mode': 'avg'})
